...
impl GradingToneTransform {
    pub fn create(style: GradingStyle) -> Result<Self> {
        let handle = unsafe { ocio_sys::ocio_grading_tone_transform_create() };
    ...
    pub fn set_value(&self, value: &GradingTone) {
        ...
        unsafe { ocio_sys::ocio_grading_tone_transform_set_value(self.handle.as_ptr(), flat.as_ptr() as *mut c_void); }
    }
