...
impl GradingHueCurveTransform {
    pub fn create(style: GradingStyle) -> Result<Self> {
        let handle = unsafe { ocio_sys::ocio_grading_hue_curve_transform_create() };
    ...
    pub fn slope(&self, curve_type: RGBCurveType, index: i32) -> f32 {
        unsafe { ocio_sys::ocio_grading_hue_curve_transform_get_slope(self.handle.as_ptr(), curve_type as i32, index as usize) }
    }

    pub fn set_slope(&self, curve_type: RGBCurveType, index: i32, slope: f32) {
        unsafe { ocio_sys::ocio_grading_hue_curve_transform_set_slope(self.handle.as_ptr(), curve_type as i32, index as usize, slope); }
    }
