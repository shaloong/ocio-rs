...
// Generated by build_support/compatibility.rs. It deliberately keeps the
// Cargo-selected Interface separate from the compiled native Implementation.
#include "ocio_rs_compatibility.h"
...
#else
  try {
#if OCIO_RS_NATIVE_API_HEX >= 0x02050000
    ocio_rs_bridge::get_real_config(handle)->clearSharedViews();
#else
    auto config = ocio_rs_bridge::get_real_config(handle);
    std::vector<std::string> views;
    const int count = config->getNumViews(ocio::VIEW_SHARED, nullptr);
    views.reserve(static_cast<size_t>(count));
    for (int index = 0; index < count; ++index) {
      const char* view = config->getView(ocio::VIEW_SHARED, nullptr, index);
      if (view && *view) views.emplace_back(view);
    }
    for (auto it = views.rbegin(); it != views.rend(); ++it) {
      config->removeSharedView(it->c_str());
    }
#endif
  } catch (...) { ocio_rs_bridge::capture_current_exception(); return; }
#endif
...
bool ocio_gpu_shader_desc_add_texture(
  ...
  if (hasBindingIndex) *hasBindingIndex = false;
  (void)bindingIndex;
  return true;
#else
  try {
    ...
    if (!values || len < expected) return false;
#if OCIO_RS_NATIVE_API_HEX >= 0x02050000
    const auto result = ocio_rs_bridge::get_real_gpu_shader_desc(handle)->addTexture(
      textureName,
      samplerName,
      width,
      height,
      ...
      values);
    if (hasBindingIndex) *hasBindingIndex = true;
...
bool ocio_gpu_shader_desc_add3d_texture(
  ...
  try {
    ...
    if (!values || len < expected) return false;
#if OCIO_RS_NATIVE_API_HEX >= 0x02050000
    const auto result = ocio_rs_bridge::get_real_gpu_shader_desc(handle)->add3DTexture(
      textureName,
      samplerName,
      edgeLen,
      static_cast<ocio::Interpolation>(interpolation),
