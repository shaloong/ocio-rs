...
// API version gate for the compiled-against OpenColorIO. Stub mode has no
// OCIO headers and fakes the full surface, so it gates as "newest".
// Guards read: #if OCIO_RS_API_HEX >= 0x02050000  (API added in OCIO 2.5)
#ifdef OCIO_RS_STUB
#define OCIO_RS_API_HEX 0x7FFFFFFF
#else
#define OCIO_RS_API_HEX OCIO_VERSION_HEX
#endif
...
#else
    out->buffer_offset = 0;  // no uniform-buffer support before OCIO 2.5
#endif
    out->value_count = 0;
    switch (data.m_type) {
...
bool ocio_gpu_shader_desc_add_uniform_vector_float(
  ...
  try {
    ...
    return ocio_rs_bridge::get_real_gpu_shader_desc(handle)->addUniform(
      ...
      maxSize);
#else
    // No max-size/uniform-buffer parameter before OCIO 2.5.
    return ocio_rs_bridge::get_real_gpu_shader_desc(handle)->addUniform(
      name,
      [owned]() { return static_cast<int>(owned->size()); },
      [owned]() { return owned->empty() ? nullptr : owned->data(); });
...
uint32_t ocio_gpu_shader_desc_add_texture(
  ...
  try {
    ...
    return ocio_rs_bridge::get_real_gpu_shader_desc(handle)->addTexture(
      ...
      values);
#else
    // addTexture() returns the shader binding index only since OCIO 2.5.
...
#else
    out->binding_index = 0;  // no shader binding indexes before OCIO 2.5
#endif
...
uint32_t ocio_gpu_shader_desc_add3d_texture(
  ...
  try {
    ...
    return ocio_rs_bridge::get_real_gpu_shader_desc(handle)->add3DTexture(
      ...
      values);
#else
    // add3DTexture() returns the shader binding index only since OCIO 2.5.
...
#ifndef OCIO_RS_STUB
// GpuLanguage enumerator values differ across OCIO versions (2.5 inserted
// GPU_LANGUAGE_GLSL_VK_4_6 mid-enum, shifting everything after it). The
// Rust-visible ABI uses the 2.5 numbering; translate by enumerator name so
// the compiled-against OCIO's numbering never leaks through the bridge.
#if OCIO_RS_API_HEX >= 0x02050000
static_assert(static_cast<int>(ocio::GPU_LANGUAGE_GLSL_VK_4_6) == 4,
...
static int ocio_rs_gpu_language_to_abi(ocio::GpuLanguage lang) {
  switch (lang) {
    ...
    case ocio::GPU_LANGUAGE_GLSL_1_2: return 1;
    case ocio::GPU_LANGUAGE_GLSL_1_3: return 2;
    case ocio::GPU_LANGUAGE_GLSL_4_0: return 3;
#if OCIO_RS_API_HEX >= 0x02050000
    case ocio::GPU_LANGUAGE_GLSL_VK_4_6: return 4;
#endif
    case ocio::GPU_LANGUAGE_HLSL_SM_5_0: return 5;
    case ocio::LANGUAGE_OSL_1: return 6;
    case ocio::GPU_LANGUAGE_GLSL_ES_1_0: return 7;
    case ocio::GPU_LANGUAGE_GLSL_ES_3_0: return 8;
    case ocio::GPU_LANGUAGE_MSL_2_0: return 9;
    // Values appended after the 2.5 numbering pass through unchanged.
    default: return static_cast<int>(lang);
  }
}
static ocio::GpuLanguage ocio_rs_gpu_language_from_abi(int lang) {
  switch (lang) {
    case 0: return ocio::GPU_LANGUAGE_CG;
    case 1: return ocio::GPU_LANGUAGE_GLSL_1_2;
    case 2: return ocio::GPU_LANGUAGE_GLSL_1_3;
    case 3: return ocio::GPU_LANGUAGE_GLSL_4_0;
#if OCIO_RS_API_HEX >= 0x02050000
    case 4: return ocio::GPU_LANGUAGE_GLSL_VK_4_6;
#endif
    // case 4 on OCIO < 2.5 is unreachable: the Rust GlslVk4_6 variant is
    // gated behind the v2_5 feature, which requires OCIO >= 2.5.
    case 5: return ocio::GPU_LANGUAGE_HLSL_SM_5_0;
    case 6: return ocio::LANGUAGE_OSL_1;
    case 7: return ocio::GPU_LANGUAGE_GLSL_ES_1_0;
    case 8: return ocio::GPU_LANGUAGE_GLSL_ES_3_0;
    case 9: return ocio::GPU_LANGUAGE_MSL_2_0;
    // Values appended after the 2.5 numbering pass through unchanged.
    default: return static_cast<ocio::GpuLanguage>(lang);
  }
}
#endif  // !OCIO_RS_STUB
...
int ocio_gpu_shader_desc_get_language(void* handle) {
  ...
  return 0;
#else
  try { return ocio_rs_gpu_language_to_abi(ocio_rs_bridge::get_real_gpu_shader_desc(handle)->getLanguage()); }
  catch (...) { ocio_rs_bridge::capture_current_exception(); return 0; }
#endif
}
...
#else
  try { ocio_rs_bridge::get_real_gpu_shader_desc(handle)->setLanguage(ocio_rs_gpu_language_from_abi(language)); }
  catch (...) { ocio_rs_bridge::capture_current_exception(); }
#endif
}
...
#else
    // Before OCIO 2.5 the parameter and texture declarations were a single
    // argument; concatenate to preserve the 2.5-shaped C API.
    std::string declarations;
    if (shaderParameterDeclarations) declarations += shaderParameterDeclarations;
    if (shaderTextureDeclarations) declarations += shaderTextureDeclarations;
    ocio_rs_bridge::get_real_gpu_shader_desc(handle)->createShaderText(
      declarations.c_str(),
      shaderHelperMethods,
      shaderFunctionHeader,
